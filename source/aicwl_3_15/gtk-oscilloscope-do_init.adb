...
procedure Do_Init
  (Widget : not null access Gtk_Oscilloscope_Record'Class;
   Lower_Sweeper, Upper_Sweeper : access Gtk_Waveform_Sweeper_Record'Class;
...
begin
   ...
   Widget.all.Values_Axis (Left  ).Justify_X := Right;
   Widget.all.Values_Axis (Middle).Justify_X := Left;
   Widget.all.Values_Axis (Right ).Justify_X := Left;
   if Lower_Sweeper = null then
      Gtk_New (Widget.all.Time_Axis (Lower).Sweeper);
   else
   ...
   if Upper_Sweeper = null then
      Gtk_New (Widget.all.Time_Axis (Upper).Sweeper);
   else
   ...
   Gtk_New (Widget.all.Group_Names, (0 => GType_String));
   --
   ...
   if Refresh_Engine = null then
      Widget.all.Refresh_Engine := new Layered_Refresh_Engine;
