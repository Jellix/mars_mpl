...
package body Gtk.Gauge.Round_90 is
   ...
   Background_Color  : constant Gdk.Color.Gdk_Color := Gtk.Missed.RGB (0.2, 0.2, 0.2);
   Line_Color        : constant Gdk.Color.Gdk_Color := Gtk.Missed.RGB (1.0, 1.0, 1.0);
   ...
   procedure Create_Background
     (Widget  : not null access Gtk_Gauge_Round_90_Record'Class;
      Sectors : Positive) is
   begin
      ...
      Widget.all.Minor_Ticks :=
        Gtk.Layered.Elliptic_Scale.Add_Elliptic_Scale
          (Under   => Widget.all.Background.all.Get_Foreground,
           ...
           Width   => 1.0 / 400.0,
           Skipped => 5,
           Step    => Length / (10.0 * Gdouble (Widget.all.Sectors)),
           From    => First,
           Length  => Length,
           Scaled  => True,
           Widened => True);
   ...
   overriding procedure Style_Changed
     (Widget : not null access Gtk_Gauge_Round_90_Record) is
   begin
