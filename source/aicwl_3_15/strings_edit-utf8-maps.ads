...
package Strings_Edit.UTF8.Maps is
   ...
   -- =, >, <, <=, >= -- Comparisons
   --
   overriding function "=" (Left, Right : Unicode_Set) return Boolean;
   ...
   function Choose
     (Set       : Unicode_Set;
      Indicator : Unicode_Indicator_Function) return Unicode_Set;
   ...
   function Is_Subset
     (Elements : Code_Points_Range;
      Set      : Unicode_Set) return Boolean renames "<=";

   function Is_Subset
     (Elements : Code_Points_Ranges;
      Set      : Unicode_Set) return Boolean;
   ...
   function Trim
     (Source : String;
      Blanks : Unicode_Set) return String;
   ...
   function Is_Prefix
     (Prefix : String;
      Source : String;
      Map    : Unicode_Mapping) return Boolean;
   ...
   function Value
     (Map     : Unicode_Mapping;
      Element : Character) return UTF8_Code_Point;

   function Value
     (Map     : Unicode_Mapping;
      Element : Wide_Character) return UTF8_Code_Point;
...
private
   ...
   overriding procedure Finalize (List : in out Code_Points_List);
   ...
   function Find
     (List : Code_Points_List;
      Code : UTF8_Code_Point) return Integer;
   ...
   --    List - The list to search through
   --
   procedure Release (Set : in out Code_Points_List_Ptr) with Inline;
   ...
   overriding procedure Adjust (Set : in out Unicode_Set);

   overriding procedure Finalize (Set : in out Unicode_Set);
   ...
   function To_Domain (Map : Map_Implementation) return String is abstract;

   function To_Range (Map : Map_Implementation) return String is abstract;

   function Value
     (Map     : Map_Implementation;
      Element : UTF8_Code_Point) return UTF8_Code_Point is abstract;

   type Map_Implementation_Ptr is access Map_Implementation'Class;

   overriding procedure Finalize (Map : in out Map_Implementation);

   procedure Release  (Map : in out Map_Implementation_Ptr) with Inline;
   ...
   overriding function To_Domain (Map : Code_Points_Map) return String;

   overriding function To_Range  (Map : Code_Points_Map) return String;
   ...
   overriding function To_Domain (Map : Code_Points_Function) return String;

   overriding function To_Range  (Map : Code_Points_Function) return String;
   ...
   overriding procedure Adjust (Map : in out Unicode_Mapping);

   overriding procedure Finalize (Map : in out Unicode_Mapping);
